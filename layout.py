...
def extract_bounding_boxes(layout_predictions, is_sparse=False):
    ...
    for layout_result in layout_predictions:
        for layout_box in layout_result.bboxes:
            label = layout_box.label if (is_sparse and not (layout_box.label == 'Table')) else 'Text'
        ...
        for region in final_regions:
            ...
            if longest_side > 0:
                ...
                try:
                    ...
                    # canvas_size = int(longest_side * 1.5) if width > height else longest_side + rescale_factor * shortest_side * 0.5
                    ...
                    # Replace pil_image with the canvased image
                    pil_image = canvas
            ...
            if label == 'a':  # Table cell
                file_name = rest[0]  # Use the filename generated in process_table_cells
            else:
