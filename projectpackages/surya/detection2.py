...
from surya.settings2 import settings2
...
def get_batch_size():
    batch_size = settings2.DETECTOR_BATCH_SIZE
    if batch_size is None:
        batch_size = 6
        if settings2.TORCH_DEVICE_MODEL == "mps":
            batch_size = 8
        if settings2.TORCH_DEVICE_MODEL == "cuda":
            batch_size = 36
    return batch_size
...
def batch_text_detection2(images: List, model, processor, batch_size=None) -> List[TextDetectionResult]:
    preds, orig_sizes = batch_detection(images, model, processor, batch_size=batch_size)
    results = []
    if settings2.IN_STREAMLIT or len(images) < settings2.DETECTOR_MIN_PARALLEL_THRESH: # Ensures we don't parallelize with streamlit, or with very few images
        for i in range(len(images)):
            result = parallel_get_lines(preds[i], orig_sizes[i])
            results.append(result)
    else:
        max_workers = min(settings2.DETECTOR_POSTPROCESSING_CPU_WORKERS, len(images))
